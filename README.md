...
배포: https://nile27.github.io/task-board-assignment/
...
### Priority 1 (필수 구현) — **전부 구현 완료** ✅
1. **로드 상태 처리:** 데이터 로딩 중, 에러 발생 시 재시도, 빈 상태(등록된 작업 없음) UI 명확히 분리
2. **낙관적 업데이트 및 실패 롤백:** 이동/수정/삭제/생성 시 즉시 화면에 먼저 반영하고 서버 요청 진행. 실패 시 전체 재조회가 아닌 "실패한 해당 카드만 이전 스냅샷으로 롤백"하여 UX 및 성능 최적화
3. **경쟁 상태(Race Condition) 처리:** 빠른 연속 이동 시 요청 직렬화(디바운스+클린업 기반)를 통해 최종 상태가 서버와 일치하도록 정합성 보장
4. **대량 데이터 성능 (5,000개):** `@tanstack/react-virtual`을 도입하여 5,000개의 카드가 렌더링되어도 드래그 및 화면 렌더링이 버벅이지 않도록 최적화
5. **태스크 관리(CRUD):** 추가, 수정, 삭제(다이얼로그 확인 후) 모두 완성
6. **핵심 로직 유닛 테스트:** `lib/tasks.ts`에서 롤백 및 상태 조작 로직을 순수 함수로 분리하여 테스트 코드(`tasks.test.ts`) 작성 및 검증 완료

### Priority 2 (권장 구현) — **미구현** ❌
- 409 충돌 처리 UX (충돌 자체는 감지 후 최신 상태 덮어쓰기로 해소했으나, 세밀한 UX는 미구현)
- 실패한 요청의 재시도 / 백오프 로직
- 다중 탭 동기화
- 키보드 접근성 향상
...
### 💡 미구현 사유
- 주어진 시간적 제약 내에 **Priority 1(필수 요구사항)의 안정성을 완벽하게 다지는 것이 더 중요하다고 판단**했습니다.
- 부가적인 UX 기능보다는 견고한 프론트엔드 상태 설계(낙관적 업데이트 로직, 대량 데이터 렌더링 최적화, 데이터 정합성 유지)를 단단하게 구현하는 데 집중하여, Priority 2 항목들은 우선순위를 미뤄 미구현으로 남겨두었습니다.

## 🛠 사용 기술 스택
- **Core:** React 18, TypeScript, Vite
- **Data Fetching & State:** React 기본 훅 (`useState`, `useRef`, `useEffect`) 기반 직접 구현
- **Virtualization:** `@tanstack/react-virtual` (대량 데이터 렌더링 최적화)
- **Test:** Vitest
